 ...
 *
 * The latest `viewedId`/`isBusy` are read through an effect event so the subscription is set up ONCE
 * on mount (not torn down and rebuilt every render), and removed on unmount.
 *
...
export function useConversationBroadcast({
  ...
  const handleConversationBroadcast = useEffectEvent((event: ConversationBroadcastEvent): void => {
    switch (event.type) {
      case 'list-changed':
  ...
  useEffect(() => {
    return conversationsBroadcaster.subscribe((event) => handleConversationBroadcast(event));
  }, [queryClient]);
}
