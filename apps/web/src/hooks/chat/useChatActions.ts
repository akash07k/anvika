...
export interface ChatActionsOptions {
  /** Whether a generation is in flight (Stop is available only then). */
  busy: boolean;
 ...
 * `useReadinessLog`). Each handler is memoized with `useCallback` so its identity is
  ...
  const afterSendGate = useCallback(
    (run: () => Promise<void>) => {
      ...
      void proceed.then(run).catch((err) => {
        ...
        // so surface it here rather than dropping it silently. Network/server errors are caught
