...
/**
 * Map a provider `APICallError`'s HTTP status to a short, content-safe category with a next step. Reads
 ...
 * provider/API error (detected by CLASS via `APICallError.isInstance`) is mapped to a specific
 ...
 *   params; when `true` a 400 response explains which local-server setting to change. Defaults to `false`.
 * @returns A content-safe category message.
 */
export function safeChatErrorMessage(error: unknown, localThinkingParamsSent = false): string {
  ...
  // still maps to its specific category instead of the generic default.
