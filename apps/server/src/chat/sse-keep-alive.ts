 ...
 * real chunk, so a comment is only ever emitted during a quiet period (never mid-event
    ...
    start(controller) {
      ...
      const pump = async (): Promise<void> => {
        try {
          ...
          clear();
          // Surface an upstream error only while the output is still writable. If the consumer
