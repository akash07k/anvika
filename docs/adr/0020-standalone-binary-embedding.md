...
- Making the binary independent of its working directory requires replacing all three file-reading seams with embedded
...
- **`--bytecode` flag**: rejected. Bytecode compilation only speeds startup, which a
  boot-once server does not benefit from. It inflates binary size and has seen less use than the normal
