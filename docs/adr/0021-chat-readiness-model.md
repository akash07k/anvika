...
- A brand-new user who has never entered a key should see a welcome panel with setup steps
...
- **No gate (prior behavior)** - rejected. Without a gate the composer is always present but
  ...
  and no way to recover. This is the experience the feature exists to fix.
...
- **Persisted `firstRunComplete` flag** - rejected. A boolean flag in settings adds
  ...
  whenever the app is unconfigured (key removed, settings reset), which a one-time
  flag would prevent.
...
- The welcome panel reappears automatically whenever the app returns to an
