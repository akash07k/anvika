...
chat model (for example `gpt-4o`) is unsupported; depending on the model
...
(content-safe). They are BILLED as output tokens; a 12k budget can add noticeable cost
and latency.
...
Why this avoids invalid requests and provider-specific branching:
...
- Future-proof: a new reasoning model is one new entry in `REASONING_RULES` (or simply
  ...
  `startsWith('gpt-5')`).
- Safe fallback: unmatched pairs return `{ supported: false }`, so no invalid option object is
...
- Reasoning text is RESPONSE CONTENT. It must never be logged by default, exactly like
  prompt and response text (see the logging standard in `docs/agents/logging.md`).
