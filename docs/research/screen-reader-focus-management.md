...
3. The reliable answer is **not a single DOM trick - it is "focus + announce" combined**: move
...
- The most reliable approach does **not rely on the browse cursor following focus at all** - fire an
...
## Techniques to move SR focus to content, ranked by reliability (NVDA-weighted)
...
2. **Make the announcement the load-bearing channel, focus the positioning channel.** On
   ...
   user-tested dual approach and continues to work when NVDA's "browse mode follows focus" setting is
