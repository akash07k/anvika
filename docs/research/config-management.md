...
- Server: a Zod v4 schema-driven configuration module. The schema drives validation, defaults, TypeScript types, and (via `z.toJSONSchema()`) settings-UI rendering hints.
...
- `z.record(z.enum([...]), v)` is EXHAUSTIVE in v4 (all keys required); `.default(map)` supplies a
  complete map. Use `z.partialRecord` only for optional keyed maps.
